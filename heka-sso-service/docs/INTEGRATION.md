...
*Why:* turns the demoable MVP into something operable in real deployments — the best-UX and most-secure login path (DC API), and clean session termination. (Three items were postponed out of this phase on 2026-08-26: WebSocket push → P3.7, polling suffices for now and the built implementation was reverted; runtime client/login-config management → P3.5, env-based configuration suffices for now; additional credential formats → P3.6, the demo SD-JWT credential suffices for now.)
