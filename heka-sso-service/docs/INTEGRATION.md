...
- [ ] **OP core**: async provider factory — `features.devInteractions: false`, `cookies.keys`, `jwks` (RS256/ES256 + `kid`), `ttl`, `renderError`, `provider.proxy = true`, discovery/`/authorize`/`/token`/`/jwks`/`/userinfo`, code flow + PKCE, both `client_secret_basic` and `client_secret_post`, `clockTolerance` for Keycloak's 0s skew. Mounted at the app root.
