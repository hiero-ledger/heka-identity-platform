...
2. **Complete**: the wallet responds **directly to heka-identity-service** (`direct_post` / DC API verify endpoint), which validates signature, holder binding, nonce, and status. The login page learns of completion via WebSocket push (polling fallback) — target design; until P3.7 lands, polling is the implemented channel.
...
What the user sees and does, with the machinery they never see marked *(hidden)*. Companion to the feasibility doc's technical sequence (§3.3 there); this is the P1.6 implementation: QR + polling (P1.6.3 — DC API landed with P2.1; WebSocket push postponed to P3.7).
...
*Why:* turns the demoable MVP into something operable in real deployments — the best-UX and most-secure login path (DC API), and clean session termination. (Four items were postponed out of this phase: WebSocket push → P3.7, polling suffices for now and the built implementation was reverted; runtime client/login-config management → P3.5, env-based configuration suffices for now; additional credential formats → P3.6, the demo SD-JWT credential suffices for now; extra `sub` strategies → P3.8, the `derived` default covers all current logins.)
...
- [ ] **P2.10 — Bridge-page branding & customization** (decision recorded 2026-08-25). The bridge serves four HTML documents, in two categories: the **wallet login page** (`src/oidc/login-page.ts` — fully static since P2.1.1, a pure JSON-API client) and three **provider hooks** in `provider.factory.ts` (`logoutSource` — must embed the XSRF form verbatim, `postLogoutSuccessSource`, `renderError` — all server-rendered by library contract). Two hard boundaries frame all branding work: the interaction pages **stay on the bridge origin** (§3.3 cookie binding; the P2.1.1 other-origin rejection must not be resurrected via branding, iframes included — clickjacking/cookie partitioning), and the three hooks **remain server callbacks** — so "client-side" means moving the *pixels* out of TypeScript, not the pages off the backend. Phased plan:
  - **P2.10.1 — File templates + shared static assets** (do first, cheap): extract the four documents from template literals into `.html` files (loaded at boot, `{{form}}`/`{{host}}`-style placeholders) and serve one shared stylesheet + logo from a bridge path (e.g. `/interaction/assets/*` — a carve-out in the root-mount dispatch or `@nestjs/serve-static`; nest-cli asset copying in the build). Branding becomes editing HTML/CSS — volume-mountable in Docker for per-deployment branding without a rebuild.
  - **P2.10.2 — Login page becomes a built front-end artifact** (when P2.9-style styling lands): a small Vite-built page (vanilla TS or Preact — React is heavy for one screen) in its own folder (`ui/` or a sibling package), built to static files the bridge serves from its own origin. Lintable/testable source instead of an inline string; SCSS with the platform's variables/mixins and design tokens shared with heka-identity-service-web-ui (the P2.1.1 reuse rule: shared pixels, not semantics). Bundle must stay self-contained — no CDN references on a login page; cost: a second build stage in Docker/CI. Only the login page joins — the three hooks stay thin branded shells linking the same built CSS (the logout confirm is auto-submitted in the broker chain and barely visible; their branding value is one stylesheet).
  - **P2.10.3 — Per-login-config `branding` block** (the §4.2 theming hook, the actual per-customer mechanism): logo, colors (CSS variables), product name, optional custom CSS in the login configuration, exposed to the page (e.g. via `GET /interaction/:uid/data`), so one bridge deployment renders per-client branding. Becomes redeploy-free once P3.5 makes login configs DB-backed.
...
- [ ] **P3.7 — WebSocket push** to the login page (moved from P2.2): subscribe to identity-service verification events, relay to the page over a same-origin cookie-bound socket; polling stays the automatic fallback. A complete, tested implementation existed and was reverted at postponement time — re-implement from the blueprint and rationale preserved under the postponed **P2.2** entry (incl. P2.2.1 push-vs-polling and P2.2.2 WebSocket-vs-SSE; SSE remains a valid simplification for the downstream leg, since a plain GET stream can reuse `interactionDetails` for the §3.3 binding instead of hand-verifying the cookie). Dependencies to restore: `ws` (+ `keygrip` for the upgrade cookie check, if the WebSocket variant is kept); deployment note: the reverse proxy must forward upgrades for `/interaction/*/events`, and a proxy that doesn't degrades to polling, not breakage. Revisit when login-page latency (~2 s poll tick) or polling load (a Postgres `interactionDetails` lookup + an identity-service REST call per page per 2 s) matters in a real deployment.
