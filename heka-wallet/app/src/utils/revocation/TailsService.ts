...
import { AgentContext, FileSystem, TypedArrayEncoder, InjectionSymbols } from '@credo-ts/core'
    ...
    try {
      ...
      if (!tailsExists) {
        ...
        await fileSystem.downloadToFile(tailsLocation, tailsFilePath, {
          verifyHash: {
            algorithm: 'sha256',
            hash: TypedArrayEncoder.fromBase58(options.revocationRegistryDefinition.value.tailsHash),
          },
        })
